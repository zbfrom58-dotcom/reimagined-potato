...
async def generate_and_send(account_key, message):
    ...
    persona = store.effective_persona(account_key, cfg.persona)
...
def register_handlers(account_key, client):
    ...
    async def incoming(event):
        if not store.is_enabled(account_key):
            return
        if not store.effective_is_allowed(account_key, event.chat_id):
            return
        ...
        if is_automatic_channel_forward(message):
            if not store.effective_autocomment_enabled(account_key):
                return
            if not store.bump_and_should_comment(account_key, event.chat_id):
                return
        else:
            if not store.effective_reply_enabled(account_key, event.chat_id):
                return
