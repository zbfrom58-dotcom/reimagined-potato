...
# Personal mode is the normal/default behavior.
# Common mode becomes active only when its master switch is ON.
COMMON_DEFAULTS = {
    "mode": "personal",
    "persona": "",
    "reply_interval": 1,
...
def set_autocomment_enabled(account_key: str, value: bool):
    d = _load(account_key)
    d["autocomment_enabled"] = bool(value)
    _save(account_key, d)
...
def get_mode() -> str:
    mode = _common_load().get("mode", "personal")
    return mode if mode in {"common", "personal"} else "personal"
...
def is_common_mode() -> bool:
    # IMPORTANT:
    # "common" in the stored radio-button setting is not enough.
    # The global master switch must also be ON. Otherwise every account
    # must continue working independently in personal mode.
    d = _common_load()
    return d.get("mode") == "common" and bool(d.get("enabled", False))
...
def bump_and_should_comment(account_key: str, chat_id: int) -> bool:
    return _bump(
        ...
        chat_id,
        "autocomment_counters",
        get_autocomment_interval(account_key),
    )
